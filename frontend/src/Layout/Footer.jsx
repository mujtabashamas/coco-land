...
const Footer = ({selectedUser, setMessages}) => {
    ...
    const handleSendMessage = () => {
        ...
        if (message.trim() || media) {
            ...
            // setMessages((prevItem) => [...prevItem, {selectedUser: selectedUser, message: chatMessage}])
            // dispatch(sendMessage({ user: selectedUser, message }));

            setMessage('');
                    
            // dispatch(sendMessage({ user: selectedUser.id, message: chatMessage }));
            // setMessage('');
            // setMedia(null);
        }
    };
        ...
        <footer className='bg-lightBrown h-[8rem]'>
            <div className="container pt-5 mx-auto space-y-10">
                <div className="flex flex-row items-center justify-between space-x-5">
                    <button className="bg-yellow-100 border border-black px-2 py-1 font-bold">Premium</button>
                    ...
                    <div className="flex items-center w-full border border-black px-2 h-10 bg-white relative">
                        ...
                        <input
                            ...
                            placeholder={media ? "Type a message or press Enter to send..." : "Type a message..."}
                        />
                    </div>
                    ...
                    <input 
                        ...
                        id="mediaUpload"
                    />
                    <label htmlFor="mediaUpload" className="bg-green-200 border border-black px-3 py-1 font-bold cursor-pointer">
                        Media
                    </label>
                    <button className="relative bg-blue-200 border border-black px-3 py-1 font-bold" onClick={() => setShowEmojiPicker(!showEmojiPicker)}>
                        Smiley
                        {showEmojiPicker && (
